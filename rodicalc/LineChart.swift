    ...
    */
    ...
    private var drawingHeight: CGFloat = 0 {
        didSet {
            ...
            y.linear = LinearScale(domain: [min, max], range: [min, drawingHeight])
    ...
    private func drawDataDots(lineIndex: Int) {
        ...
        for index in 0..<data.count {
            ...
            dotLayer.frame = CGRect(x: xValue+10, y: yValue, width: dots.outerRadius, height: dots.outerRadius)
    ...
    private func drawAxes() {
        ...
        // draw x-axis
        x.axis.color = UIColor.blueColor()
        path.moveToPoint(CGPoint(x: x.axis.inset+5, y: height-15))
        path.addLineToPoint(CGPoint(x: width - x.axis.inset+5, y: height-15))
        path.stroke()
        // draw y-axis
    ...
    private func getMinimumValue() -> CGFloat {
        ...
        var min: CGFloat = 1500
        for data in dataStore {
            let tmp = data.minElement()!
            if tmp < min {
                min = tmp
            }
        }
    ...
    private func drawXLabels() {
        ...
        for (index, _) in xAxisData.enumerate() {
            ...
            let label = UILabel(frame: CGRect(x: xValue+10, y: y+5, width: width, height: x.axis.inset))
