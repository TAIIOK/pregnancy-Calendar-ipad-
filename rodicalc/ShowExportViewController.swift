...
class ShowExportViewController: UIViewController , UIScrollViewDelegate  {
    ...
    func create_text(notes: [TextNoteE], notifi: [TextNoteE], type: Int)-> [NSMutableAttributedString]{
        ...
        for(var n = 0 ; n < notes.count ; n++)
        {
            ...
            switch type {
            case 0:
                count += Int(lroundf(Float(Float(characters)/143)))
                val = 22
            case 1:
                count += Int(lroundf(Float(Float(characters)/86)))
                val = 20
            case 2:
                count += Int(lroundf(Float(Float(characters)/87)))
                val = 21
            case 3:
                count += Int(lroundf(Float(Float(characters)/60)))
                val = 21
            default:
                break
            }
            ...
            if count > val{
                ...
                if attrText.length > 0 {
                    ...
                    print("type",type)
                    if type != 0 && type != 1{
                        returnedText.append(NSMutableAttributedString(string: ""))
                    }
                }
